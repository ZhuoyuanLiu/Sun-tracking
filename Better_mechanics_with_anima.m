...
%%% there should be a yaw rotation, and a roll rotation. Moreover, the rotation in hour-angle axis is not entirely a rotation along a fixed
%%% axis, the strings in that axis will also manipulate the roll angle, and
%%% with the gravity of the panel, the yaw angle will also change
%%% -Yet to be done.
%%% -Added yaw change, to be adaptive
...
actual_yaw_angle = 60; % Actual Yaw rotation angle (*Roll, not yaw)
% Initial yaw angle
actual_initial_yaw = -30;           % To make adaptive based on string mechanics. The yaw change doesn't seem to affect the accuray though
...
for i = 1:numFrames     % Consider daylight time is 12 hours (between sunrise and set. Adjust to adaptive-done)
    ...
    actual_currentRoll = actual_roll_angle * (i / numFrames);
    actual_currentR_axis = R_axis(currentTheta);
    actual_currentR_roll = R_roll(-actual_currentRoll + actual_initial_roll);
    ...
    % Apply the changing yaw rotation
    actual_rotatedVertices = (R_yaw(actual_currentYaw) * vertices')';
    ...
    actual_rotatedVertices = (actual_currentR_roll * actual_rotatedVertices')';
