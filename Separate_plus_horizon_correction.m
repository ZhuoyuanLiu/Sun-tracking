...
%%% there should be a yaw rotation, and a roll rotation.    -Yet to be done
%%% Calculate accuracy -done

%%% Two configurations:
%%% 1. Rotate the cross around its z-axis   -Not yet proceeded
%%% 2. Rotate the pitch motor back to north -done and verified
clear;
...
R_pitch = @(pitch) [1 0 0; 0 cosd(pitch) -sind(pitch); 0 sind(pitch) cosd(pitch)];      % Euler rotation: Z-X-Z
...
offset_pitch = tension_coefficient * 90;    % For the deformation angle,The base value should be rethought of based on the strings mechanic, but now we are not using this.         
%offset_pitch = 2*Dec_sample;                % Adjust to adaptive-done
...
for i = 1:numFrames     % Consider daylight time is 12 hours (between sunrise and set. Adjust to adaptive-done. Now frame scale is 0.01*daylight hours)
    ...
    if (i<=numFrames/2)         % Sun is by east
        current_pitch = initial_actual_pitch_angle-offset_pitch* (i / numFrames) + correction_pitch*close_to_horizon - correction_pitch*(correction_frame / (0.3*100/2) );
        ...
        current_pitch = initial_actual_pitch_angle-offset_pitch/2+offset_pitch* ( (i-numFrames/2) / numFrames) + correction_pitch*(correction_frame / left_frames);
    end
    ...
    % if YES, then perform horizon correction.
        ...
        if close_to_horizon
            ...
            correction_pitch = -offset_pitch/2;      % Adjust to adaptive-done - thought: how many degrees at most will it deviate to the north from east or from west at a certain date?
                                        % Before adjustment (-60), mean efficiency=0.9297
                                        % After adjustment, mean efficiency=0.9281
...
output_energy_day_2nd(day) = daylight_hour*effciency(day)*Max_KW;
consumed_energy_day_2nd(day) = KWH_deg * (theta_change/2 + abs(correction_pitch)*2/2 + 2*(max(DEC_num)-min(DEC_num))/365/2 );
end % End yearly

figure;
subplot(3, 1, 1);
plot(1:365, output_energy_day_2nd, '-r', 'DisplayName', 'output');
xlabel('day');
ylabel('Energy (KWH)');
title('Output energy');
legend show;
grid on;
subplot(3, 1, 2);
plot(1:365, consumed_energy_day_2nd, '-b', 'DisplayName', 'Consumed');
xlabel('day');
ylabel('Energy (KWH)');
...
grid on;
subplot(3, 1, 3);
plot(1:365, output_energy_day_2nd-consumed_energy_day_2nd, '-b', 'DisplayName', 'Net');
xlabel('Frame');
ylabel('Net energy');
title('Net energy');
legend show;
grid on;
mean(consumed_energy_day_2nd)
mean(output_energy_day_2nd)
