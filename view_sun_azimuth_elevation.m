...
% Take samples to test. This is the middle of the year, around september 1st
...
theta_change = 15*12.83; % Sun's motion across the sky. *Valid for horizontal movement, not valid for hour angle change....
...
numFrames = 240; % 0.1 hour per frame
...
roll_angle = -180+(90-Dec_sample+42);   % Use declination relationship. SUN's noon elevation = 90-Dec+polar height
...
for i = 1:numFrames     % Consider daylight time is 12 hours (between sunrise and set)
    ...
    %Initial_theta = 90;    
    Initial_theta = -(90 + RA_sample);     % Use hour angle and RA relationships, set initial theta to be the 
