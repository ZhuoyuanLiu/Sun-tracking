...
mean_DEC= mean(DEC_num);
median_DEC = median(DEC_num);
...
for day = 1:365
    ...
    ideal_Dec_sample = DEC_num(day); 
    Dec_sample = median_DEC;  
...
%          [axisPoint(3) axisPoint(3) + axisDirection(3) * 2], 'Color', 'red', 'LineWidth', 2);
    ...
    ideal_pitch_angle = 90-(180-(90-ideal_Dec_sample+42));   % Use declination relationship. SUN's noon elevation = 90-Dec+polar height
    ...
    % a year
    tension_coefficient = 1+Dec_sample/100;     % When the panel at solar noon is tilted more, 
                                                ...
                                                % make less accuray
    coe(day) = tension_coefficient;
    % offset_pitch = 2*Dec_sample;   % The base value should be rethought of         
    offset_pitch = tension_coefficient * 90;
    offset_pitch = 2*(Dec_sample+actual_pitch_angle);
    ...
    for i = 1:numFrames     % Consider daylight time is 12 hours (between sunrise and set)
        ...
        % This part is to simulate the process of the panel going from east (string-deformed) into
        % center point and to west (string-deformed again)
        if (i<=numFrames/2)
...
%              [axisPoint(3) axisPoint(3) + axisDirection(3) * 2], 'Color', 'red', 'LineWidth', 2);
    ...
    if (azimuth_angles(1)>0 && azimuth_angles(numFrames)>0)
        azimuth_change(day) = abs( azimuth_angles(1) - azimuth_angles(numFrames) );
    else
        azimuth_change(day) = 360 - ( abs( azimuth_angles(1) - azimuth_angles(numFrames) ) );
    end
    elevation_change(day) = 2*( max( abs(elevation_angles) )-min( abs(elevation_angles) ) );
...
%     close(gcf);
    ...
    output_energy_day_0th(day) = daylight_hour*effciency(day)*Max_KW;
    consumed_energy_day_0th(day) = KWH_deg * (theta_change/2);
end
% Display the normal vectors over time
figure(1)
subplot(3, 1, 1);
plot(1:365, output_energy_day_0th, '-r', 'DisplayName', 'output');
xlabel('day');
ylabel('Energy (KWH)');
title('Output energy');
legend show;
grid on;
subplot(3, 1, 2);
plot(1:365, consumed_energy_day_0th, '-b', 'DisplayName', 'Consumed');
xlabel('day');
ylabel('Energy (KWH)');
...
grid on;
subplot(3, 1, 3);
plot(1:365, output_energy_day_0th-consumed_energy_day_0th, '-b', 'DisplayName', 'Net');
xlabel('Frame');
ylabel('Net energy');
title('Net energy');
legend show;
grid on;
mean(consumed_energy_day_0th)
mean(output_energy_day_0th)
azi_ele_consumed = KWH_deg* ( azimuth_change+elevation_change/2 );  % Azimuth *2 because motor stroke in azimuth is 1:1 not 1:2
azi_ele_consumed_mean = mean(azi_ele_consumed)

% Simulate AZI-ele design energy output 
figure(2)
azi_ele_output_energy_day = daylight_decimal_hours*Max_KW;
subplot(3, 1, 1);
plot(1:365, azi_ele_output_energy_day, '-r', 'DisplayName', 'output');
xlabel('day');
ylabel('Energy (KWH)');
title('azi-ele Output energy');
legend show;
grid on;
subplot(3, 1, 2);
...
xlabel('day');
ylabel('Energy (KWH)');
title('azi-ele Consumed energy');
legend show;
grid on;
subplot(3, 1, 3);
plot(1:365, azi_ele_output_energy_day'-azi_ele_consumed, '-b', 'DisplayName', 'Net');
xlabel('Frame');
ylabel('Net energy');
title('azi-ele Net energy');
legend show;
grid on;
mean(azi_ele_consumed)
mean(azi_ele_output_energy_day)
mean(azi_ele_output_energy_day'-azi_ele_consumed)
