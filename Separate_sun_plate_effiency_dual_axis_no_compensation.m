...
%          [axisPoint(3) axisPoint(3) + axisDirection(3) * 2], 'Color', 'red', 'LineWidth', 2);
    ...
    offset_pitch = tension_coefficient * 90;    % The base value should be rethought of
    %offset_pitch = 2*Dec_sample; 
...
%              [axisPoint(3) axisPoint(3) + axisDirection(3) * 2], 'Color', 'red', 'LineWidth', 2);
    ...
    output_energy_day_1st(day) = daylight_hour*effciency(day)*Max_KW;
    consumed_energy_day_1st(day) = KWH_deg * ( theta_change/2 + 2*(max(DEC_num)-min(DEC_num))/365/2 );
end
figure;
subplot(3, 1, 1);
plot(1:365, output_energy_day_1st, '-r', 'DisplayName', 'output');
xlabel('day');
ylabel('Energy (KWH)');
title('Output energy');
legend show;
grid on;
subplot(3, 1, 2);
plot(1:365, consumed_energy_day_1st, '-b', 'DisplayName', 'Consumed');
xlabel('day');
ylabel('Energy (KWH)');
...
grid on;
subplot(3, 1, 3);
plot(1:365, output_energy_day_1st-consumed_energy_day_1st, '-b', 'DisplayName', 'Net');
xlabel('Frame');
ylabel('Net energy');
title('Net energy');
legend show;
...
mean(consumed_energy_day_1st)
mean(output_energy_day_1st)
